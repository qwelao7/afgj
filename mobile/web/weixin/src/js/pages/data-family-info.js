...
$(function () {
    ...
    $(document).on("pageInit", "#data-family-info", function (e, id, page) {
        ...
        function loadFamily() {
            var info = JSON.parse(window.localStorage.getItem('data_owner'));
            info.house = window.localStorage.getItem('data_detail');
        ...
        $(document).on('click', '.tag_group_extra', function () {
            var id = url.id;
            window.location.href = 'data-label-add.html?id=' + id;
        });


        //返回
        $(page).on('click', '#back', function () {
            var id = window.localStorage.getItem('data_roomCode');
            window.location.href = 'data-family.html?id=' + id;
        });

        /** 搜索标签 **/
        $(document).on('focus', 'input[name=search]', function () {
            window.location.href = 'data-tag-search.html?id=' + url.id;
        });
