...
$(function () {
    ...
    $(document).on("pageInit", "#event-create", function (e, id, page) {
        ...
        /** 报名截止 **/
        $('#end-siginup').datetimePicker({
            onClose: function() {
                pickerValidate('#end-siginup', 'input[name=apply_end]');
            }
        });
         ...
         * 渲染友元选择
         */
        function renderPoints(ids) {
            //render  init
            if (ids == pointsIds) {
                $('input[name=point_checkbox]').prop('checked', true);
            } else if (ids == []) {
                $('input[name=point_checkbox]').prop('checked', false);
            }

            $('input[name=accept_point_community_id]').val(ids.join(','));
            checkArr = ids;
        ...
        function add(params) {
            common.ajax('POST', '/events/create', params, true, function(rsp) {
                if (rsp.data.code == 0) {
                    $.alert('活动创建成功!', '提交成功', function () {
                        location.href = 'event-create-success.html';
                    })
                } else {
