<?php
namespace common\components\pinyin;
/**
 * @name Pinyin_Pinyin
 * @desc 汉字转拼音 (utf8)
 * @author 张顺(zhangshun@baidu.com)
 */
use common\components\pinyin\ChinesePinyinTable;
        ...
        foreach ($arrStringList as $string) {
            switch (strlen($string)) {
                case 1:
                    ...
                    break;
                case 3:
                    if (isset(ChinesePinyinTable::$arrChinesePinyinTable[$string])) {
                        $arrResult[] =
                            ChinesePinyinTable::$arrChinesePinyinTable[$string];
                    } else {
                        $arrResult[] = array($string);
                    }
                    break;
